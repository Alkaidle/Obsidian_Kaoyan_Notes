...
**选/不选**
```c++
void dfs(int i, vector<int>& candidates, int left) {
        if (left==0) {
            ans.push_back(path);
            return;
        }
...
 }
```
![[Pasted image 20260207222221.png]]
至多的方案数
`ans = sum(dp[0...bag])`
至少的方案数
`ans = sum(dp[bag ... S]), S为总和`
##### 状态机dp
状态之间转换关系的图称为状态机
- **不限交易次数**
[122. 买卖股票的最佳时机 II](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-ii/description/)
![[Pasted image 20260301172105.png]]
![[Pasted image 20260301172416.png]]
- **至多k次**
新增一维，表示剩余交易次数
<u>空间优化</u>：变为二维， 类似01背包，倒序
![[Pasted image 20260301204210.png]]
##### 区间DP
小区间->大区间
正序or倒序：**划分子问题**，**写出状态转移方程**，看转移过程中是从大到小转移(倒序)还是从小到大(正序)，区间两个端点转移顺序通常相反
#### 单调栈
第一个比当前数小/大的数
- 及时去掉无用数据，保证栈中数据有序
#### 单调队列
某个区间的最大/最小值
最大：`deque` 队头为最大值，入队元素若比队尾元素大，则该区间内队尾元素不可能为最大值，直接出队
```c++
...
#### Top K
- 堆-只找TopK
前k个元素生成一个小根堆，存当前最大的k个元素
从k+1开始和堆顶比较，大于堆顶则替换，并调整堆
遍历完后小根堆中剩余k-1个元素都比堆顶大
#### 哈希
```c++
unordered_set<> / unordered_map<> // 无序，插入O(1)
set<> / map<> // 有序，插入O(logn)
```
![[Pasted image 20260311164212.png]]常见哈希键设计：
- 在字符串和数组当中，当每个元素的顺序不重要时，可以使用**排序后的字符串或数组作为键**
  ```c++
        for (auto& str: strs) {
...
        }
  ```
- 如果只关心每个值的偏移量，例如第一个值的偏移量，则可以使用**偏移量作为键**
- 在树中，我们通常会用**子树的序列化表述作为键**